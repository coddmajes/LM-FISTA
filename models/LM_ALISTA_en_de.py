...
class LM_ALISTA_en_de(tf.Module):
    def __init__(self, _layers, _A, _W, _unshared, _nums, _ss, _percent, _max_percent, _na, _en_de, _D_cl2num, _classify_type, _classes, _Vscope,
                 _lam=1., soft=None):
        super(LM_ALISTA_en_de, self).__init__()
        """
            _layers: int
            _A: matrix
    ...
    def __call__(self, in_b):
        ...
        for i in range(self.lays):
            ...
            if i == 0:
                if self.has_ss:
                    percent = self._ps[i]
                    x = shrink_ss(alpha * tf.matmul(self._Wt_, in_b), T, percent)
                else:
                    x = shrink_free(alpha * tf.matmul(self._Wt_, in_b), T)
            else:
                # keep t(k)
                gamma = self.gamma
                z = x + gamma * (x - xs)
                ...
                if self.has_ss:
                    percent = self._ps[i]
                    x = shrink_ss(z + alpha * tf.matmul(self._Wt_, res), T, percent)
                else:
                    x = shrink_free(z + alpha * tf.matmul(self._Wt_, res), T)
