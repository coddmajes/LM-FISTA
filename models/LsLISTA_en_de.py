...
class LsLISTA_en_de(tf.Module):
    def __init__(self, _layers, _A, _unshared, _nums, _ss, _percent, _max_percent, _na, _en_de, _D_cl2num, _classify_type, _classes, _Vscope,
                 _lam, soft=None):
        super(LsLISTA_en_de, self).__init__()
        """
            _layers: int
            _A: matrix
    ...
    def __call__(self, in_b):
        ...
        if self.is_na:
            ...
            for i in range(self.lays):
                ...
                if self.has_ss:
                    percent = self._ps[i]
                    x = shrink_ss(x + tf.matmul(W * step, res), lam * step, percent)
                else:
                    x = self.soft(x + tf.matmul(W * step, res), lam * step)
