    ...
    def __call__(self, in_b):
        ...
        for i in range(self.lays):
            ...
            if i == 0:
                if self.has_ss:
                    percent = self._ps[i]
                    x = shrink_ss(tf.matmul(W , in_b), theta, percent)
                else:
                    x = self.soft(tf.matmul(W , in_b), theta)
            else:
                # keep t(k)
                gamma = self.gamma
                z = x + gamma * (x - xs)
                ...
                if self.has_ss:
                    percent = self._ps[i]
                    x = shrink_ss(z + tf.matmul(W, res), theta, percent)
                else:
                    x = self.soft(z + tf.matmul(W, res), theta)
